...
## 2026-09-25 — Kolesa Flags: без перезаписи всей таблицы и без сброса флагов при tarpit (t-0039)

### Fixed
- **Churn:** `parsers/kolesa/flags.py` каждые 8ч делал baseline-UPDATE всех active
  kolesa (~370k строк), даже если флаг не менялся. Каждая строка → новая версия
  кортежа + записи во все 13 индексов listings: главный вклад в 3–4-кратное
  раздутие индексов и переполнение Neon 512 MB. Плюс `flags_updated_at = NOW()` у
  всех строк заставлял инкрементальный синк архива тянуть всю таблицу (egress Neon).
  Теперь UPDATE трогает только строки, где значение реально меняется
  (`IS DISTINCT FROM`); повторный прогон с теми же ID не пишет ничего.
- **Флаги «очищались» при неполном сборе:** kolesa тарпитит GHA, сбор фильтров
  `?need-repair=1` / `?auto-custom=1` обрывается на ~20 из ~170 страниц (сегодня:
  355 из ~3,400 аварийных), а baseline сбрасывал флаги у ВСЕХ несобранных —
  тысячи аварийных/нерастаможенных машин попадали в ценовую статистику как чистые.
  Теперь `collect_filter_ids` возвращает признак полноты; сброс только при полном
  проходе (естественный конец пагинации), иначе флаги только добавляются (липкий
  TRUE). 0 ID при «полном» проходе с 1-й страницы считается неполным (смена разметки).

### Changed
- `flags_updated_at` = «когда флаги строки последний раз менялись» (бэкенд его не читает).
- Статистика прогона: `baseline_marked` = сброшено к чистому состоянию
  (+ `emergency_reset`, `customs_reset`, `*_complete`).

### Added
- `tests/test_flags_collect.py` (4 теста: естественный конец / tarpit / пустая
  1-я страница / MAX_PAGES); логика UPDATE проверена на локальной копии схемы.

## 2026-09-25 — Парсеры снова упёрлись в Neon 512 MB: починка архивного контура + спул (t-0039)

### Fixed
- **Корень:** launchd-агенты архивного контура (2026-08-29) указывали прямо в репо
  в `~/Documents`, а macOS TCC запрещает фоновым процессам читать `~/Documents` —
  каждую ночь `Operation not permitted` (exit 126), молча, 27 дней подряд. Синк и
  подрезка Neon ни разу не отработали, база доросла до 512 MB, и с ~2026-09-20 все
  пишущие workflow падали с `DiskFullError`: Daily Parsers (deactivate), Kolesa
  Flags (запись флагов), Kolesa Full Parse (каждое сохранение — `не удалось
  сохранить`), `parser_runs` не писался с 22.09, ни одной точки цены с ~20.09.
- Ручной синк Neon → `kolesa_archive` (архив: 740k → 898k объявлений, 934k → 1.11M
  точек цены) и подрезка: из Neon убраны 148,247 объявлений с `last_seen_at` старше
  90 дней — все подтверждённо в архиве (rejected 0). В Neon осталось 415,206.
- Подрезка сама по себе не помогла вставкам цен: лимит Neon считает РАЗМЕР ФАЙЛОВ,
  VACUUM их не уменьшает, а индексы с растущим ключом требуют новых страниц.
  `neon_compact.sql` (REINDEX CONCURRENTLY, без блокировок): кластер **512 → 363 MB**,
  индексы были раздуты в 3–4 раза (`listings_source_id_external_id_key` 47 → 13 MB,
  `listings_pkey` 32 → 13, `idx_price_history_listing` 26 → 14).
- **Тихие крэши лёгких парсеров:** mycar/newauto/avtorynok/olx ловили исключение в
  `__main__`, слали Telegram и выходили с кодом 0 — job зелёный, хотя парсер упал на
  первом же `DiskFullError` (mycar в догоняющем прогоне 2026-09-24). Теперь `sys.exit(1)`.
- mycar/newauto/olx: сбой записи одного объявления больше не обрывает прогон/фид —
  warning и дальше (объявление уже в спуле), как давно сделано в kolesa/avtorynok.

### Added
- `infrastructure/archive/install.sh` — ставит контур так, чтобы launchd мог его
  исполнять: скрипты → `~/.local/share/kolesa-archive`, `DATABASE_URL` →
  `~/.config/kolesa-archive/neon.env` (600); один агент `com.kolesa.archive-nightly`
  (ежедневно 03:30: синк — вс full, иначе incr ради egress-квоты Neon — + подрезка)
  вместо двух старых. Итог → `~/Library/Logs/kolesa-archive/last-status`, при сбое —
  уведомление macOS. Проверено `--run-now` через сам launchd: `ok`, exit 0.
- `prune_neon.sh`: бюджет `NEON_MAX_LISTINGS` (450k) поверх `HOT_DAYS` — сверх бюджета
  уходят самые давно не виденные (но не свежее `MIN_HOT_DAYS`=30); подрезка ежедневно,
  а не раз в неделю; `PRUNE_SYNC_MODE=incr|full`.
- **Спул несохранённых объявлений** (`parsers/common/spool.py`): если `save_listing`
  не смог записать по причине БД — объявление уходит в `spool/*.jsonl` с моментом
  наблюдения; `kolesa_full.yml` и `daily_parsers.yml` заливают его артефактом
  `spool-*` (30 дней). Ошибки данных (`DataError`, `IntegrityConstraintViolation`)
  в спул не идут. Итог — `::warning::`-аннотация на прогоне.
- `save_listing(..., data["seen_at"])`: запись наблюдения задним числом — `last_seen_at`
  не откатывается назад (GREATEST), `is_active` не воскрешается, если объявление
  деактивировали после наблюдения, `price_history.recorded_at` = момент наблюдения,
  цена сравнивается с действовавшей тогда. Без `seen_at` поведение прежнее (NOW()).
- `parsers/common/replay_spool.py` + **workflow `db_guard.yml`** (каждые 3ч):
  проверка ёмкости Neon (`neon_capacity.py`: сумма `pg_database_size` по кластеру,
  ≥ 470 MB → Telegram «Neon почти полон») и долив артефактов спула в БД; удаляются
  только полностью долитые, при недоступной БД артефакты остаются.
- `tests/test_spool.py` (5 тестов); интеграционно проверено на локальной копии схемы:
  обычная запись без изменений, спул при обрыве, идемпотентный долив со старым `seen_at`.

### Removed
- `com.kolesa.archive-sync.plist`, `com.kolesa.archive-prune.plist` (заменены
  шаблоном `com.kolesa.archive-nightly.plist` + `install.sh`).

### Impact
- Neon снова принимает запись: после компакта идут новые объявления, точки цен и
  `parser_runs`. Потерянное за 20–25.09 не восстановить: до спула оно не сохранялось.
- Не связано с БД и НЕ исправлено здесь: OLX с раннеров GHA получает HTTP 403 почти на
  каждый запрос (1–6 удачных страниц за прогон с 11.09, 0 — с 19.09); kolesa по-прежнему
  тарпитит GHA (~9k объявлений за прогон). Оба упираются в IP датацентра → t-0040.
- Повторное переполнение теперь (1) маловероятно — подрезка ежедневно + бюджет строк,
  (2) заметно за дни — Telegram-алерт с 470 MB и уведомление macOS при сбое ночного
  прогона, (3) не теряет данные — спул + автодолив.
- После правок `infrastructure/archive/*` нужно перезапускать `install.sh`.
...
### Fixed
- **58 прогонов подряд `kolesa_full.yml` отменялись по 350-мин timeout** (последний успех — до июля). Причина не в объёме работы: kolesa.kz перешла на tarpit — с ~7-й минуты (~50 страниц на IP шарда) все соединения виснут до 30с-таймаута вместо 403. `parse_city` глотал таймауты как «ошибки фида» (5 подряд → тихий break), детектор IP-блока `consecutive_full_fails` не срабатывал, и job 5.5 часов жёг ретраи (~15 мин на фид × 46 фидов ≈ 12.5ч > 350 мин).
- Теперь фид, умерший не отдав **ни одной** страницы, поднимает `TarpitError` (подкласс `IPBlockedError`) → существующий детектор прерывает прогон: exit 1, deactivate пропущен, спарсенное до бана сохранено. Ожидаемая длительность забаненного прогона ~30–40 мин вместо 5ч50м отмены.
- Env-переменные `PARSER_REQUEST_DELAY_MIN/MAX` и `PARSER_MAX_RETRIES`, которые workflow задавал с апреля, **молча игнорировались кодом** (задержка жёстко 2–4с, ретраи жёстко 3): теперь читаются в `http_client.py` и `kolesa/parser.py`.
- Косметика: лог ретраев показывал «попытка 4/3» — знаменатель теперь учитывает первую попытку (4/4).

### Added
- **Ротация фидов между прогонами**: список фидов циклически сдвигается на `(GITHUB_RUN_NUMBER × 37) % 297` до шардирования. Раньше при хроническом бане свежий `last_seen_at` получали только первые фиды списка (almaty, karaganda, …) — остальные протухали неделями. Здоровый прогон покрывает все фиды как и раньше (перестановка того же множества); локальные запуски без ротации.
- Новая env `PARSER_HTTP_TIMEOUT` (default 30с) — таймаут одного HTTP-запроса; в `kolesa_full.yml` установлена 15с (штатный ответ kolesa ~1–2с). Вместе с `PARSER_MAX_RETRIES: 2` цена мёртвой страницы при tarpit падает с ~3 мин до ~1 мин → бан подтверждается за ~15 мин.

### Impact
- Забаненные прогоны завершаются быстро и со статусом `failure` (🔴 в Telegram) вместо бесконечных `cancelled` — виден реальный статус, а данные до бана попадают в БД.
- Давление на анти-бот kolesa снижается: ~2760 мёртвых connection-attempts за прогон (ретраи по всем фидам) → ~90.
- Отдельная проблема того же периода — Neon 512MB (`could not extend file`, все сохранения падали) — решается в t-0037 (стопгэп: дроп индексов; архивный контур на Mac mini).

---

## 2026-07-05 — Ops: первая архивация выполнена, миграции применены, прод задеплоен

### Changed
- Применены миграции 002/003/004 в Neon (listings_archive, price_history_archive, parser_runs, parse_cursor).
- Выполнена первая архивация: **228,701 listings + 275,784 price_history** перенесены из горячих таблиц. Горячие таблицы теперь: listings 348,365 (336,644 active), price_history 449,051.
- Обнаружен лимит Neon free tier в действии: bulk-прогон упёрся в 512 MB на пачке 24 (DiskFullError) — внутрипроектный архив не освобождает место сам по себе. Процедура пересмотрена: архив = staging-буфер → локальный дамп (`archive-dumps/*.csv.gz`, 4 файла, CSV-сверка построчно) → TRUNCATE буферов → VACUUM горячих таблиц. Подробности в README §Архивация.
- Прод передеплоен вручную через Render Dashboard (GitHub→Render интеграция сломана с 26 апреля, см. t-0035).

### Impact
- В горячих таблицах снова есть месяцы headroom под вставки парсеров (VACUUM освободил ~130 MB под переиспользование).
- Repo variable `ARCHIVE_DRY_RUN` сознательно НЕ установлена в 0: еженедельный real-run без автоматизации выгрузки снова упрётся в лимит.
...
- **/analytics/forecast 500 на любых параметрах (t-0020):** в `analytics.py` не был импортирован `timedelta` (используется в `_is_holiday_week` и генерации дат прогноза) → NameError при n≥4 недель. Страница «Прогноз» не работала целиком. Плюс UI теперь различает 5xx («Сервис прогноза временно недоступен») и реальную нехватку данных.
- **POST /auth/register 500 (t-0021):** passlib 1.7.4 несовместим с bcrypt≥4.1 (удалён `bcrypt.__about__`) → `hash_password` падал. Запинен `bcrypt==4.0.1`. Регистрация → логин → /me проверены end-to-end.
- **Backend игнорировал `model_id` (t-0023):** добавлен параметр в `/summary`, `/market-overview`, `/recent`; `/geo` получил `brand_id`/`model_id`/`year`. Раньше при выбранной модели KPI показывали числа всей марки, карта — всего рынка (555k при KPI 97k).

### Added
- **Auth подключена к UI (t-0022):** кнопка «Войти» в топбаре, user-меню с email/планом и «Выйти» (по `/auth/me`), страницы /auth/* теперь достижимы.
- **Поиск ⌘K (t-0024):** новая палитра `SearchPalette` — fuzzy по маркам и «марка модель», Enter/клик ставит фильтр дашборда через store. Раньше кнопка «Поиск» была мёртвой (aria-label честно говорил «не реализовано»).
- **Внутренние ссылки (t-0034):** лента ведёт на /listing?id= (внешний сайт — иконкой ↗), топ-таблица моделей — на /model?brand=&model= (market-overview теперь возвращает `slug`). Страницы /listing и /model перестали быть сиротами.

### Fixed (data pipeline)
- **Мусорные модели (t-0026):** централизованная санитизация в `save_listing` (`_sanitize_model_name`): «camry тоета камри на разбор» → «Camry», slug пересчитывается → сливается с каноническим. Лечит mycar/avtorynok/olx на будущее; существующие данные мигрируются отдельно.
- **OLX без брендов (t-0027):** разметка OLX сменилась — title переехал в `<h4>`, первым `<p>` стала цена; парсер писал цену в title → 97% объявлений без марки («Без названия» в ленте). Селекторы переписаны на классификацию по содержимому; словарь брендов дополнен народными названиями («нива», «волга», «камри»…), джанк-префиксы расширены.
- **Дубль Костаная в /geo (t-0028, часть):** слаги-синонимы (kostanay/kostanai, uralsk/oral) агрегируются по display-имени, города с 0 объявлений выкидываются из ответа.

### Fixed (UI)
- Hydration errors #418/#423/#425 (t-0032): «ОБНОВЛЕНО HH:MM» в FilterBar рендерился на билде и не совпадал с клиентом — теперь ставится после mount.
- KPI USD/KZT «▼ +0.5%» (t-0029): знак задаёт стрелка, число без знака. Заголовок KPI меняется на «Все объявления» в режиме «Все». Дельта цены в ленте скрывается при 0 (было «· —» в каждой строке). Подпись количества у боксплота не обрезается краем («15 78.»). «31 городов» → правильное склонение. Поповер настроек прижат к шестерёнке (был в правом нижнем углу).
- Капитализация брендов (t-0030): `fmt.brandName` — BMW/BYD/JAC/MG/GAC/FAW/SsangYong/Lynk & Co/Mercedes-Benz во всех таблицах, селектах, каталоге.
- Карта (t-0031): подписи топ-3 городов разводятся top/bottom (не перекрываются), клик по городу ставит фильтр через store не затирая остальные (t-0025; раньше перезаписывал URL и ничего не применял), светлая тема переключает тайлы на light_nolabels.
- Города в ленте — русские названия («Алматы» вместо «almaty») через `fmt.cityName` (t-0033).
- Каталог марок: скрыты марки с 0 активных, «234 марок» → «N марки/марок» (t-0028/t-0029).

### Impact
- Все 3 критикала закрыты: прогноз работает, регистрация работает, ссылки шарятся.
- Модельная аналитика (KPI/карта/лента) больше не врёт при выбранной модели.
- Новые OLX-объявления будут с брендами; мусорные модели перестанут плодиться.
