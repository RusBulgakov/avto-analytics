...
- **Kolesa anti-bot из GHA:** kolesa.kz агрессивно блокирует burst запросов из Azure-датацентра. Эмпирически 4 шарда × 3 concurrent (12 одновременных) → IP блок за ~10 минут; ~80 req/min ловит накопительный бан через ~4ч. Конфигурация: 3 шарда × 2 concurrent (6 одновременных) + delay 4–7с (~66 req/min). Не повышайте rate/concurrency — потеряете весь прогон. **2026-08: kolesa перешла на tarpit** — вместо 403 каждое соединение виснет до таймаута; бан наступает уже на ~7-й минуте (~50 страниц на IP шарда) даже при фактических ~22 req/min. Из-за этого 58 прогонов подряд (июль–август) отменялись по 350-мин timeout: каждый мёртвый фид выжигал ~15 минут ретраев (5 ошибок × 4 попытки × 30с), полный список — ~12.5ч. Фикс: парсер распознаёт tarpit (фид умер, не отдав ни одной страницы → `TarpitError`, подкласс `IPBlockedError`) → срабатывает детектор `consecutive_full_fails` → прогон завершается exit 1 за ~40 минут с сохранением спарсенного (deactivate пропускается). Фиды ротируются между прогонами по `GITHUB_RUN_NUMBER`, чтобы «окно до бана» каждый раз доставалось другим фидам, а не вечно первым по списку. Env-ручки `PARSER_REQUEST_DELAY_MIN/MAX`, `PARSER_MAX_RETRIES` и новая `PARSER_HTTP_TIMEOUT` теперь реально читаются кодом (до 2026-08 молча игнорировались: задержка была жёстко 2–4с, ретраи 3, таймаут 30с).
...
- **Kolesa Flags (`parsers/kolesa/flags.py`):** флаги «аварийная» / «не растаможена» берутся из search-фильтров kolesa. UPDATE пишет только строки, где флаг меняется (до 2026-09-25 каждые 8ч переписывались все ~370k active — раздувание Neon). Сброс флагов у несобранных объявлений — только если фильтр пройден до конца; под tarpit'ом сбор обрывается на ~20-й странице, и тогда флаги лишь добавляются (липкий TRUE), чтобы аварийные машины не попадали в ценовую статистику как чистые.
